...
class AliceDataModule(L.LightningDataModule):
    ...
    def __init__(self, data_dir: str = '', background_file: str = '', background_xs = 1.6270497, signal_component = msq.Component.SIG, background_component = msq.Component.BKG, scaler_path = 'scaler.pkl', sample_size = 10000, offset=0, batch_size: int = 32, random_state: int=None) -> None:
        super().__init__()
    ...
    def prepare_data(self):
        self.data_full = AliceDataset(os.path.join(self.data_dir, self.background_file),
                                      self.background_xs,
...
class AliceDataset(Dataset):
    ...
    def __init__(self, background_file: str = '', background_xs = 1.6270497, signal_component = msq.Component.SIG, background_component = msq.Component.BKG, sample_size = 10000, offset = 0, random_state = None):
        super().__init__()
    ...
    def __create_dataset(self):
        sample_background = sample.from_csv(cross_section=self.background_xs, file_path=self.background_file, n_rows=self.offset+self.sample_size*1.2)
        ...
        sample_processed = sample_background.filter(bkg_null_filter).filter(bkg_nan_filter).calculate(z_cand).filter(z_masses).calculate(angles).calculate(four_lepton_vars)[self.offset:self.offset+self.sample_size]
        ...
        pdf_ratios = sample_processed.probabilities/sample_processed.reweight(numerator=self.signal_component, denominator=self.background_component).probabilities

        y = (1/(1 + pdf_ratios)).to_numpy()

        # Use N*(p(theta_1) + p(theta_0))/2 as sample weights
        sample_weights = self.sample_size*(sample_processed.probabilities + sample_processed.reweight(numerator=self.signal_component, denominator=self.background_component).probabilities).to_numpy()/2
    ...
    def copy(self):
        ds = object.__new__(AliceDataset)
        ds.background_file = self.background_file
        ds.background_xs = self.background_xs
        ds.signal_component = self.signal_component
        ds.background_component = self.background_component
