...
class Modifier():
  ...
  def __init__(self, baseline, events, c6_points = [-20, -10, 0, 10, 20], ct_values = [-1,0,1], cg_values = [-0.01, 0.0, 0.01]):
  ...
  def modify(self, c6 = None, ct = None, cg = None):
    c6_powers = np.stack([np.power(cH,i) for i in range(self.c6_degree+1)], axis=0)   # (5, Nx)
