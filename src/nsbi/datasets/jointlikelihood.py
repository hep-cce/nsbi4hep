...
class AliceDataModule(L.LightningDataModule):
    def __init__(
        ...
        numerator_component=msq.Component.SIG,
        denominator_component=msq.Component.BKG,
    ...
    def setup(self, stage: str):
        if stage == "fit":
            events_train, events_val = self.events.shuffle(random_state=self.random_state).split(
                train_size=0.5, val_size=0.5
            )

            if self.c6_points is None:
                self.training_data = JointLikelihoodDataset(
                    events_train,
                ...
                self.validation_data = JointLikelihoodDataset(
                    events_val,
                    features=self.features,
                    numerator_component=self.numerator_component,
                    denominator_component=self.denominator_component,
                ...
                self.training_data = JointLikelihoodParameterizedDataset(
                    events_train,
                    features=self.features,
                    c6_points=self.c6_points,
                    numerator_component=self.numerator_component,
                    denominator_component=self.denominator_component,
                    sample_size=self.sample_size,
                    random_state=self.random_state,
                )
                self.validation_data = JointLikelihoodParameterizedDataset(
                    events_val,
                    features=self.features,
                    c6_points=self.c6_points,
                    numerator_component=self.numerator_component,
...
class JointLikelihoodDataset(Dataset):
    def __init__(
        ...
        r = (
            ...
            / events.reweight(
                numerator=numerator_component, denominator=denominator_component
            ).probabilities
        )
    ...
    def __getitem__(self, index):
        return torch.tensor(self.X[index], dtype=torch.float32), torch.tensor(
            self.s[index], dtype=torch.float32
        )
...
class JointLikelihoodParameterizedDataset(Dataset):
    def __init__(
        ...
        super().__init__()
        c6_mod = (
            c6.Modifier(baseline=numerator_component, events=events, c6_points=[-5, -1, 0, 1, 5])
            if numerator_component != msq.Component.INT
            else c6.Modifier(baseline=numerator_component, events=events, c6_points=[-5, 0, 5])
        )
        ...
        sample_weights = pd.Series(
            (probabilities_numerator + probabilities_denominator) / 2 * sample_size
        ).reset_index(drop=True)

        unweighted_indices = sample_weights.sample(
            n=sample_size, replace=True, weights=sample_weights, random_state=random_state
        ).index
