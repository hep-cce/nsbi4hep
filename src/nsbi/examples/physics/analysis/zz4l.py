...
class ZPairCandidate:
    ...
    def find_Z_lsq(self, l1, l2, l3, l4):
        ...
        pairs = vector.array(
            [[p12, p34], [p14, p23]],
            dtype=[("px", np.float64), ("py", np.float64), ("pz", np.float64), ("E", np.float64)],
        )
        lepton_pairs = vector.array(
            [[[l1, l2], [l3, l4]], [[l1, l4], [l3, l2]]],
            dtype=[("px", np.float64), ("py", np.float64), ("pz", np.float64), ("E", np.float64)],
        )

        # Squared minimization to determine the closest pair
        sq = np.array([
            (pair[0].mass - self.Z_mass) ** 2 + (pair[1].mass - self.Z_mass) ** 2 for pair in pairs
        ]).T
        closest_pair_indices = np.argmin(sq, axis=1)
        closest_pair = pairs.transpose(2, 0, 1)[
            np.arange(len(closest_pair_indices)), closest_pair_indices
        ].T
        ...
        l1_1, l2_1 = lepton_pairs.transpose(3, 0, 1, 2)[
            np.arange(len(closest_pair_indices)), closest_pair_indices
        ][np.arange(len(pT_max_ind)), pT_max_ind].T
        l1_2, l2_2 = lepton_pairs.transpose(3, 0, 1, 2)[
            np.arange(len(closest_pair_indices)), closest_pair_indices
        ][np.arange(len(pT_min_ind)), pT_min_ind].T
        ...
        return {
            ...
            "Z2_l2_pz": l2_2.pz,
            "Z2_l2_E": l2_2.E,
            "Z1_mass": (l1_1 + l2_1).mass,
            "Z2_mass": (l1_2 + l2_2).mass,
            "Z1_pt": (l1_1 + l2_1).pt,
            "Z2_pt": (l1_2 + l2_2).pt,
        }
    ...
    def find_Z_closest(self, l1, l2, l3, l4):
        ...
        pairs_diffs = (
            ((pairs.mass - np.ones(pairs.shape) * self.Z_mass) ** 2)
            .transpose(2, 0, 1)
            .reshape(pairs.shape[2], 4)
        )
        ...
        # (l1_1, l2_1) = Z1; (l1_2, l2_2) = Z2
        l1_1, l2_1 = lepton_pairs.transpose(3, 0, 1, 2)[np.arange(len(min_ind)), min_ind][
            np.arange(len(closest_Z_min_ind)), closest_Z_min_ind
        ].T
        l1_2, l2_2 = lepton_pairs.transpose(3, 0, 1, 2)[np.arange(len(min_ind)), min_ind][
            np.arange(len(closest_Z_max_ind)), closest_Z_max_ind
        ].T
...
class ZPairMassWindow:
    def __init__(self, z1: tuple[int, int] | None = None, z2: tuple[int, int] | None = None):
        self.z1 = z1
        self.z2 = z2
...
class AngularVariables:
    def __init__(self):
        ...
        self.variable_functions = {
            "cth_star": self.calc_cth_star,
            "cth_1": self.calc_cth_1,
            "cth_2": self.calc_cth_2,
            "phi_1": self.calc_phi_1,
            "phi": self.calc_phi,
            "mZ1": self.calc_mZ1,
            "mZ2": self.calc_mZ2,
        }
    ...
    def __call__(self, kinematics):
        l1 = vector.array({
            ...
            "py": kinematics["l1_py"],
            "pz": kinematics["l1_pz"],
            "E": kinematics["l1_energy"],
        })
        l2 = vector.array({
            "px": kinematics["l2_px"],
            "py": kinematics["l2_py"],
            "pz": kinematics["l2_pz"],
            "E": kinematics["l2_energy"],
        })
        l3 = vector.array({
            "px": kinematics["l3_px"],
            "py": kinematics["l3_py"],
            "pz": kinematics["l3_pz"],
            "E": kinematics["l3_energy"],
        })
        l4 = vector.array({
            "px": kinematics["l4_px"],
            "py": kinematics["l4_py"],
            "pz": kinematics["l4_pz"],
            "E": kinematics["l4_energy"],
        })
    ...
    def calc_phi_1(self, *leptons: MomentumObject4D):
        ...
        nz = vector.array({
            "x": np.zeros(Z1_h.shape[0]),
            "y": np.zeros(Z1_h.shape[0]),
            "z": np.ones(Z1_h.shape[0]),
        })

        n12 = l1_h.cross(
            l2_h
        ).unit()  # Normal vector of the plane in which the Z1 decay takes place
        nscp = nz.cross(
            z1
        ).unit()  # Normal vector of the plane in which the H -> Z1, Z2 takes place
    ...
    def calc_phi(self, *leptons: MomentumObject4D):
        ...
        n34 = l3_h.cross(
            l4_h
        ).unit()  # Normal vector of the plane in which the Z2 decay takes place
...
class LeptonMomenta:
    def __call__(self, kinematics):
        ...
        leptons_sorted = vector.array(
            np.take_along_axis(leptons, indices, axis=1),
            dtype=[("px", np.float32), ("py", np.float32), ("pz", np.float32), ("E", np.float32)],
        )
        charges_sorted = np.take_along_axis(charges, indices, axis=1)

        return {
            "l1_pt": leptons_sorted[:, 0].pt,
            "l1_eta": leptons_sorted[:, 0].eta,
            "l1_phi": leptons_sorted[:, 0].phi,
            "l1_energy": leptons_sorted[:, 0].energy,
            "l1_charge": charges_sorted[:, 0],
            "l2_pt": leptons_sorted[:, 1].pt,
            "l2_eta": leptons_sorted[:, 1].eta,
            "l2_phi": leptons_sorted[:, 1].phi,
            "l2_energy": leptons_sorted[:, 1].energy,
            "l2_charge": charges_sorted[:, 1],
            "l3_pt": leptons_sorted[:, 2].pt,
            "l3_eta": leptons_sorted[:, 2].eta,
            "l3_phi": leptons_sorted[:, 2].phi,
            "l3_energy": leptons_sorted[:, 2].energy,
            "l3_charge": charges_sorted[:, 2],
            "l4_pt": leptons_sorted[:, 3].pt,
            "l4_eta": leptons_sorted[:, 3].eta,
            "l4_phi": leptons_sorted[:, 3].phi,
            "l4_energy": leptons_sorted[:, 3].energy,
            "l4_charge": charges_sorted[:, 3],
        }
...
class FourLeptonSystem:
    def __init__(self):
        ...
        """
        self.variable_functions = {
            "4l_mass": self.calc_m4l,
            "4l_rapidity": self.calc_y4l,
            "4l_pT": self.calc_pT,
            "4l_energy": self.calc_E,
        }

    def __call__(self, kinematics):
        l1 = vector.array({
            "pt": kinematics["l1_pt"],
            "eta": kinematics["l1_eta"],
            "phi": kinematics["l1_phi"],
            "energy": kinematics["l1_energy"],
        })
        l2 = vector.array({
            "pt": kinematics["l2_pt"],
            "eta": kinematics["l2_eta"],
            "phi": kinematics["l2_phi"],
            "energy": kinematics["l2_energy"],
        })
        l3 = vector.array({
            "pt": kinematics["l3_pt"],
            "eta": kinematics["l3_eta"],
            "phi": kinematics["l3_phi"],
            "energy": kinematics["l3_energy"],
        })
        l4 = vector.array({
            "pt": kinematics["l4_pt"],
            "eta": kinematics["l4_eta"],
            "phi": kinematics["l4_phi"],
            "energy": kinematics["l4_energy"],
        })
...
def analyze(events):
    ...
    events_analyzed = (
        events_analyzed.filter(z_masses)
        .filter(LeptonPtEtaCut(1, pt_min=20, eta_max=2.5))
        .filter(LeptonPtEtaCut(2, pt_min=15, eta_max=2.5))
        .filter(LeptonPtEtaCut(3, pt_min=10, eta_max=2.5))
        .filter(LeptonPtEtaCut(4, pt_min=7, eta_max=2.5))
    )
