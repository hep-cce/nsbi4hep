...
def msq_eft_over_sm(eft_coefficients, c6=None, ct=None, cg=None):
    ...
    msq_eft_over_sm = torch.einsum(
        "nijk,ix,jy,kz->nxyz", eft_coefficients, c6_powers, ct_powers, cg_powers
    )
...
class Modifier:
    def __init__(
        ...
        *,
        ...
        X, Y, Z = np.meshgrid(
            self.c6_points, self.ct_values, self.cg_values, indexing="ij"
        )  # Shape: (5, 3, 3)
        ...
        msq_bsm = self.events.components[
            [mcfm.csv_component_bsm[self.baseline][xyz] for xyz in xyz_bsm]
        ].to_numpy()
        ...
        self.coefficients = coefficients.T.reshape(
            -1, self.c6_degree + 1, self.ct_degree + 1, self.cg_degree + 1
        )
