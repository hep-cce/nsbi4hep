...
class monitored_model(L.LightningModule):
    ...
    def __init__(self, base_module, arg_order=None,feature_names=None, **kwargs):
        ...
        self.chi2_metrics = tools.ReweightingClosureMetric(observables=[x for x in range(n_features)], binning=None, metric="chi2")
        
        self.ws_metrics = tools.ReweightingClosureMetric(observables=[x for x in range(n_features)], binning=None, metric="wasserstein")
