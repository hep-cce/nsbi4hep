...
class Modifier:
    def __init__(self, baseline, events, c6_points=(-20, -10, 0, 10, 20)):
        ...
        self.c6_components = [mcfm.csv_component_c6[self.baseline][c6_pt] for c6_pt in c6_points]

        # Matrix of shape (N_events, N_c6_points)
        msq_sm = self.events.components[mcfm.csv_component_sm[self.baseline]].to_numpy()
        msq_c6 = np.array([
            self.events.components[c6_component].to_numpy() for c6_component in self.c6_components
        ...
        rhs = np.divide(
            msq_c6,
            msq_sm[:, np.newaxis],
            out=np.ones_like(msq_c6),
            where=msq_sm[:, np.newaxis] != 0,
        )
