...
csv_components = [f"msq_{comp}_sm" for comp in ["sig", "int", "sbi", "bkg"]] + [
    f"msq_{comp}_bsm_{i}" for comp in ["sig", "int", "sbi"] for i in range(1, n_bsm_points + 1)
]

csv_component_sm = {comp: f"msq_{comp.value}_sm" for comp in Component}
...
def to_csv(self, file_path, **kwargs):
    df = pd.concat(
        [self.kinematics, self.components, self.weights.to_frame(name=csv_weight)], axis=1
    )
...
class Process:
    ...
    def filter(self, filter):
        accepted_indices = filter(
            self.kinematics, self.components, self.weights, self.probabilities
        )
    ...
    def shuffle(self, random_state=None):
        shuffled_kinematics, shuffled_components, shuffled_weights = shuffle(
            self.kinematics, self.components, self.weights, random_state=random_state
        )
        return Process(
    ...
    def split(self, train_size=1, val_size=1, test_size=None):
        if test_size is not None:
            ...
            return (
                Process(
                    kinematics_train.reset_index(drop=True),
                    components_train.reset_index(drop=True),
                    weights_train.reset_index(drop=True),
                ),
                Process(
                    kinematics_val.reset_index(drop=True),
                    components_val.reset_index(drop=True),
                    weights_val.reset_index(drop=True),
                ),
                Process(
                    kinematics_test.reset_index(drop=True),
                    components_test.reset_index(drop=True),
    ...
    def sample(self, n, random_state=None):
        ...
        sampled_events_indices = self.weights.sample(
            n, replace=False, weights=None, random_state=random_state
        ).index
    ...
    def unweight(self, n, random_state=None):
        unweighted_events_indices = self.weights.sample(
            n=n, replace=True, weights=self.weights, random_state=random_state
        ).index
        ...
        return Process(
            ...
            pd.Series(
                np.ones_like(unweighted_events_indices) * self.weights.sum() / n
    ...
    def reweight(self, denominator, numerator):
        reweights = (
            self.weights
            * self.components[csv_component_sm[numerator]]
            / self.components[csv_component_sm[denominator]]
        )
        return Process(
    ...
    def __getitem__(self, item):
        return Process(
            self.kinematics.iloc[item], self.components.iloc[item], self.weights.iloc[item]
        )
