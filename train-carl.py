...
from carl.dataset import BalancedDataModule
from carl.model import CARL
...
def main(args):
    data = BalancedDataModule(signal_file = SAMPLE_FILEPATH[args.signal_process], background_file=SAMPLE_FILEPATH[args.background_process], sample_size = args.sample_size, batch_size = args.batch_size, random_state = args.random_state)
    model = CARL(args.n_features, args.n_layers, args.n_nodes)
    trainer = Trainer(accelerator=args.accelerator, max_epochs=100)
